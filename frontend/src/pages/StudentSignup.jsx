  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post('/auth/student/signup', {
        name, roll_no: rollNo, email, password, department_id: parseInt(departmentId),
      });
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-[#F5F0E8]">
      ...
      <div className="neo-card max-w-md w-full">
        <h1 className="text-3xl font-bold uppercase mb-6 border-b-2 border-black pb-4">Student Sign Up</h1>

        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <label className="block text-xs font-bold uppercase mb-2">Full Name</label>
            <input type="text" value={name} onChange={(e) => setName(e.target.value)}
              placeholder="John Doe" className="input-field w-full" />
            {errors.name && <p className="text-red-500 text-xs mt-1 font-bold">{errors.name}</p>}
          </div>

          <div className="mb-4">
            <label className="block text-xs font-bold uppercase mb-2">Roll Number</label>
            <input type="text" value={rollNo} onChange={(e) => setRollNo(e.target.value)}
              placeholder="CS001" className="input-field w-full" />
            {errors.rollNo && <p className="text-red-500 text-xs mt-1 font-bold">{errors.rollNo}</p>}
          </div>

          <div className="mb-4">
            <label className="block text-xs font-bold uppercase mb-2">Email</label>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)}
              placeholder="your@student.edu" className="input-field w-full" />
